...
func TestResponseFromBytes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if resp.Destination != tt.wantDestination {
				t.Errorf("Destination = 0x%02x, want 0x%02x", resp.Destination, tt.wantDestination)
			}
			if resp.Source != tt.wantSource {
				t.Errorf("Source = 0x%02x, want 0x%02x", resp.Source, tt.wantSource)
			}
			if tt.wantMAC != "" {
				if got := hex.EncodeToString(resp.MAC[:]); !strings.EqualFold(got, tt.wantMAC) {
					t.Errorf("MAC = %s, want %s", got, tt.wantMAC)
				}
			}
			if tt.wantEncryptedPayload != "" {
				if got := hex.EncodeToString(resp.EncryptedPayload); !strings.EqualFold(got, tt.wantEncryptedPayload) {
...
func TestResponseDecrypt(t *testing.T) {
...
func TestResponseVerifyMAC(t *testing.T) {
	_, aliceSeed, alicePub := generateTestKeyPair(t)
	_, bobSeed, bobPub := generateTestKeyPair(t)
	_, _, carolPub := generateTestKeyPair(t)
...
func TestResponseRoundTrip(t *testing.T) {
	...
	plaintext := []byte("full round trip test response")
	encrypted, err := EncryptThenMAC(sharedAlice, plaintext)
	if err != nil {
		t.Fatalf("encrypting: %v", err)
	}
	...
	decoded, err := ResponseFromBytes(wire)
	if err != nil {
		t.Fatalf("ResponseFromBytes(): %v", err)
	}

	if decoded.Destination != resp.Destination {
		t.Errorf("Destination = 0x%02x, want 0x%02x", decoded.Destination, resp.Destination)
	}
	if decoded.Source != resp.Source {
		t.Errorf("Source = 0x%02x, want 0x%02x", decoded.Source, resp.Source)
	}
